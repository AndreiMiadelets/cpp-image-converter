...
bool SaveJPEG(const Path& file, const Image& image) {
  ...
  */
  if ((outfile = fopen(file.c_str(), "wb")) == nullptr) {
    return false;
  }
  ...
  */
  ...
  image_buffer.reserve(row_stride);

  while (cinfo.next_scanline < cinfo.image_height) {
    auto *line = image.GetLine(static_cast<int>(cinfo.next_scanline));

    for (size_t x = 0; x < image_width; ++x) {
      image_buffer[x * 3 + 0] = static_cast<char>(line[x].r);
      image_buffer[x * 3 + 1] = static_cast<char>(line[x].g);
      image_buffer[x * 3 + 2] = static_cast<char>(line[x].b);
    }
